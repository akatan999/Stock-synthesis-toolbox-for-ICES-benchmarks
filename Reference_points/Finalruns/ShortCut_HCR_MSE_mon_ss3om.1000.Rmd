---
title: "Monkfish 27.8c9a: Short-Cut MSE approach for robustness tests of harvest control rules in sex-structured models"
...
```{r,echo=F}
load(file="rdata/ftune.mon.rdata")
...
```
```{r,echo=FALSE,eval=F}
save(Fx,Ftgt,Bftgt,file="rdata/ftune.mon.rdata")
```
...
```{r}
save(stk,stka,stkr,refpts,sr,sra,spars,file="rdata/om.ss3ref.mon.rdata") 
```
...
```{r}
load("rdata/om.ss3ref.mon.rdata",verbose=T)
```
...
```{r,eval=FALSE}
save(om, oem,sdevs,iem, file="rdata/flom.mon.rda")
```

```{r,echo=FALSE}
load(file="rdata/flom.mon.rda")
```
...
load(file="rdata/mps.ftune.mon1000.rdata",verbose=T)
...
save(om,oem,arule,hcrs,df,mseargs,file="rdata/inpMP.mon.1000.rdata")
```
...
library(doParallel)
library(FLRef)
load("~/mseLite/mse_refpts/rdata/inpMP.mon.1000.rdata",verbose=T)
length(hcrs$target)
# Run in batches
...
save(stks ,file=paste0("~/mseLite/mse_refpts/rdata/mps.ftune.mon1000.rdata"))
...
nfs = length(df$Tune[-1])
tri = c(rep(".Btri.eq",each=nfs))
scenarios = c("Fmsy.eq",paste0(rep(c(df$Tune[-1]),1),tri))
...
AR.PA = tab.pf[tab.pf$`P3(B<Blim)`<0.05 ,]
# Maximum catch
