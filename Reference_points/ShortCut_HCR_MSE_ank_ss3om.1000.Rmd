---
title: "White Anglerfish 27.8c9a: Short-Cut MSE approach for robustness tests of harvest control rules in sex-structured models"
...
```{r,echo=F}
load(file="rdata/ftune.ank.rdata")

```

```{r}
Fx = c(rev(seq(0.6,1,0.025)))
...
```
```{r,echo=FALSE,eval=F}
save(Fx,Ftgt,Bftgt,file="rdata/ftune.ank.rdata")
```
...
```{r}
save(stk,stka,stkr,refpts,sr,sra,spars,file="rdata/om.ss3ref.ank.rdata") 
```
...
```{r}
load("rdata/om.ss3ref.ank.rdata",verbose=T)
```
...
```{r,eval=FALSE}
save(om, oem,sdevs,iem, file="rdata/flom.ank.rda")
```

```{r,echo=FALSE}
load(file="rdata/flom.ank.rda")
```
...
load(file="rdata/mps.ftune.ank1000.rdata",verbose=T)

mp.eqsim = stks$Fmsy.eq
...
save(om,oem,arule,hcrs,df,mseargs,file="rdata/inpMP.ank.1000.rdata")
```
...
library(doParallel)
library(FLRef)
load("~/mseLite/mse_refpts/rdata/inpMP.ank.1000.rdata",verbose=T)
length(hcrs$target)
# Run in batches
...
save(stks ,file=paste0("~/mseLite/mse_refpts/rdata/mps.ftune.ank1000.rdata"))
...
```{r, fig.height=7,fig.width=9,warning=FALSE,message=FALSE, fig.cap = paste0("Median MSE forecast horizon")}
...
```{r, fig.height=8,fig.width=10, fig.cap = paste0("MSE kobe plot Advice rules"), warning=FALSE}
