...
#  This script is now tailored on ensemble grid of 7 runs with several survey, length and age data slot. It can be modified to include more survey, length or age compositions.       
#
#                                                                                                                       #
...
if (Sys.info()["sysname"] == "Windows") {
    ...
    SS_EXE   <- "~/Max/Stock_synthesis/ss3_3.22.1/ss3.exe"
    ...
    for(j in 1:length(retro_files)){
        ...
        file.copy(paste(dir.runN, "starter.ss", sep="/"),
                  paste(dir.runN.new, "starter.ss", sep="/"), overwrite=TRUE)
                                        ...
                                        # copy other files
        file.copy(paste(dir.runN, "Lobster_ctl.ss", sep="/"),
                  paste(dir.runN.new, "Lobster_ctl.ss", sep="/"), overwrite=TRUE)
        file.copy(paste(dir.runN, "Lobster_dat.dat", sep="/"),
                  paste(dir.runN.new, "Lobster_dat.dat", sep="/"), overwrite=TRUE)
...
for(i in runs){
  ...
  results$N_Params[results$Run== i] <-   tmp$N_estimated_parameters
    ...
    ## Check Runs Test
    sspar(mfrow=c(3,3),plot.cex = 0.8)    # change based on number of fleet and length/age data
...
##############################
    ...
    save(ss3rep, file=paste0(dir = rdata,"/Lobster3a_",i,".Rdata")) 
  ...
  ##############################
    ...
    sspar(mfrow=c(2,2),plot.cex = 0.9)     # change based on number of cpue or survey
    ...
    results$MASE_cpue2[results$Run== i] <-   mase_cpueB$MASE[3]
    #results$MASE_cpue4[results$Run== i] <-   mase_cpueB$MASE[4]
    results$MASE_cpue3[results$Run== i] <-   mase_cpueB$MASE[5]
    results$MASE_cpue4[results$Run== i] <-   mase_cpueB$MASE[6]
    results$MASE_cpue5[results$Run== i] <-   mase_cpueB$MASE[7]
    ...
    sspar(mfrow=c(1,2),plot.cex = 0.7) # change based on number of fleet or survey per length or age data
...
#*********************************************************
# Save the table of diagnostic for weighting purpose (value 0 to 1) for best case selection and ensemble models
#********************************************************
...
####################################################
#### Weight_vector to be used for best case selection and ensemble models
####################################################
