...
#   Edits by David Gilljam (SLU Aqua, Oregrund)                                                                   #
#   Francesco Masnadi (CNR-IRBIM & UNIBO, Ancona)


#  This script is now tailored on ensemble grid of different models with several survey, length and age data slot. It can be modified to include more survey, length or age compositions.       
#
#                                                                                                                       #
...
#    if the user want to use different Weighting methods (i.e., merge multiple similar diagnostics into a single score)   #
...
## RATIOLIM AND TRIG (only for plotting purposes here)
RATIOLIM  = 2.333 # ratio of Btarget on Blim: e.g. if target=SSB35% and lim=SSB15% --> 35/15=2.333
RATIOTRIG = 1 # ratio of 1 on trigger percentage: e.g. if trigger=100% of target --> 1/1=1

#Example with North Sea seabass stock
STARTER_FILE  = "starter.ss"
WTATAGE_FILE  = "wtatage.ss"
PAR_FILE      = "ss3.par"
FORECAST_FILE = "forecast.ss"
CTL_FILE      = "BassIVVII.ctl"
DAT_FILE      = "BassIVVII.dat"

##You can choose between number or names for identifying the different models (example here with two models)
#runs <- c("Reference_run", paste0("Run", c(1:6)))   
runs <- c("Basecase", "Basecase_mock")  
nmodels <- 2

MC.CORES = 32
MC.SILENT = TRUE

MVLN_MC       = 5000     ## 5000 is recommended for final forecast MVLN runs and figures. 500 is fine for testing.

##
## Some system specific setups (Linux vs Windows) (specify where ss3.exe is located)
##
if (Sys.info()["sysname"] == "Windows") {

    SS_EXE   <- "~/Max/Stock_synthesis/ss3_3.23/ss3.exe"
    main.dir <- "~/Max/Commitees/ICES/WKBBASS/Northern stock"
    MC.CORES <- 7
...
dir.create("rdata_runs")
dir.create("rdata_retros")

rdata = paste0(main.dir,"/rdata_runs")
rretros = paste0(main.dir,"/rdata_retros")
...
foreach(i=seq(1:nmodels), .packages="r4ss") %dopar% {
  cat("core:", i + 1, "\n")
...
for (i in 1:length(runs)){
    ...
    SS_plots(tmp, uncertainty=T, datplot=F, forecast=T, maxyr=2026, minbthresh=0.25,fitrange = F)
}

############################################################
# Build the empty diags table for weighting the models later (more slots can be added for complex models)
############################################################
results <- data.frame(Run=runs, Convergence=NA, Total_LL=NA, N_Params=NA,
                      ...
                      Runs_test_cpue6=factor(NA, levels = c("Passed","Failed")),
                      Runs_test_cpue7=factor(NA, levels = c("Passed","Failed")),
                      Runs_test_cpue8=factor(NA, levels = c("Passed","Failed")),
    ...
    for(j in 1:length(retro_files)){
        ...
        file.copy(paste(dir.runN, "starter.ss", sep="/"),
                  paste(dir.runN.new, "starter.ss", sep="/"), overwrite=TRUE)
                                        ...
                                        # copy other files
        file.copy(paste(dir.runN, "BassIVVII.ctl", sep="/"),
                  paste(dir.runN.new, "BassIVVII.ctl", sep="/"), overwrite=TRUE)
        file.copy(paste(dir.runN, "BassIVVII.dat", sep="/"),
                  paste(dir.runN.new, "BassIVVII.dat", sep="/"), overwrite=TRUE)
        ...
        file.copy(paste(dir.runN, "wtatage.ss", sep="/"),
                  paste(dir.runN.new, "wtatage.ss", sep="/"), overwrite=TRUE)
        file.copy(paste(dir.runN, "ss3.par", sep="/"),
                  paste(dir.runN.new, "ss3.par", sep="/"), overwrite=TRUE)
...
# dir.create("Plotdiags",showWarnings = F)
    ...
    sspar(mfrow=c(1,3),plot.cex = 0.8)   # change to 3 if also Age present
    ...
    ## Check Runs Test
    sspar(mfrow=c(5,5),plot.cex = 0.8)    # change based on number of fleet and length/age data
    ...
    results$Runs_test_cpue6[results$Run== i] <-   rt.cpue$test[6]
    results$Runs_test_cpue7[results$Run== i] <-   rt.cpue$test[7]
    results$Runs_test_cpue8[results$Run== i] <-   rt.cpue$test[8]
  ...
  # Age (activate more ages if present)
...
##############################
    ...
    ##          Then we would run x simulations in parallel, 6 times,
    ##          instead of 6 simulations in parallel, x times.
    ##
    ## DG note2: Move the actual running of the retrospective out from this diagnostics
    ...
    ##           to do the retrospective simulations.
    ##
    ...
    save(retroSummary, retroModels, file=paste0(dir = rretros,"/RetroModels_",i,".Rdata"))
    
    ss3rep = retroModels[[1]]
    save(ss3rep, file=paste0(dir = rdata,"/NorthernSeaBass_",i,".Rdata")) 
  ...
  ##############################
    ...
    sspar(mfrow=c(2,4),plot.cex = 0.9)     # change based on number of cpue or survey
    ...
    results$MASE_cpue6[results$Run== i] <-   mase_cpueB$MASE[6]
    results$MASE_cpue7[results$Run== i] <-   mase_cpueB$MASE[7]
    results$MASE_cpue8[results$Run== i] <-   mase_cpueB$MASE[8]
    results$MASE_cpue9[results$Run== i] <-   mase_cpueB$MASE[9]
    ...
    sspar(mfrow=c(3,5),plot.cex = 0.7) # change based on number of fleet or survey per length or age data
    mase_len.plot <- SSplotHCxval(hccomps,add=T,subplots = "len",legendloc="topright",legend = FALSE, indexUncertainty = T,legendcex = 1)
    mase_len.plot2 <- SSplotHCxval(hccomps,add=T,subplots = "len",legendloc="topright",legend = FALSE, indexUncertainty = T,legendcex = 1, Season = 4)
  mase_age <- SSplotHCxval(hccomps,add=T,subplots = "age",legendloc="topright",legend = FALSE, indexUncertainty = T,legendcex = 1) # activate if age data is present!
    dev.print(jpeg,paste0(dir = dir.diag,"/HCxval_len_",i,".jpg"), width = 8, height = 9, res = 300, units = "in")

    ## Save in the final diags table (activate or create more if needed)
    mase_len <- SSmase(hccomps,quants = "len")
    mase_len2 <- SSmase(hccomps,quants = "len")
  ...
  # age (activate or create more if needed)
    ...
    results$MASE_age11[results$Run== i] <-   mase_age$MASE.adj[11]
...
#*********************************************************
# Save the table of diagnostic for weighting purpose (value 0 to 1) for best case selection and ensamble models
#********************************************************
...
####################################################
#### Weight_vector to be used in the ensemble
####################################################
...
##****************************************************************
## Approximate uncertainty with MVLN (hessian matrix)
##****************************************************************
...
for(i in 1:length(runs)){
    ...
    mvn.temp = SSdeltaMVLN(run, run = runs[i],
                           years = run$startyr:run$endyr,
                           addprj = TRUE, mc = MVLN_MC, weight = weight_vector[i], Fref = "MSY", plot=FALSE, verbose=TRUE)
    kbproj = rbind(kbproj,
...
SSplotKobe(kbproj, fill=T, joint=F, posterior="points", ci.levels = c(0.5, 0.95, 0.99), ylim = c(0,2.5),
           ylab=expression(F/F[MSY]),
           xlab=expression(SSB/SSB[MSY]), legendpos = "right", legend = TRUE)
dev.print(jpeg,paste0("Kobe_final_points_cv.jpg"), width = 12, height = 8, res = 300, units = "in")

sspar(mfrow=c(1,1),plot.cex = 0.7)
SSplotKobe(kbproj, fill=T, joint=F, posterior="kernel", ci.levels = c(0.5, 0.95, 0.99), ylim = c(0,2.5),
           ylab=expression(F/F[MSY]),
           xlab=expression(SSB/SSB[MSY]), legendpos = "right", legend = TRUE)
dev.print(jpeg,paste0("Kobe_final_kernel_cv.jpg"), width = 12, height = 8, res = 300, units = "in")
...
labels  <- expression(SSB/SSB[MSY], "F/F"[MSY], "SSB", "F", "Recruits", "Catch")
...
write.csv(prj_F_Btrg, "Ensemble_F_Ftrg.csv")
