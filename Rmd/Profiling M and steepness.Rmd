---
title: "WKBSEABASS Southern stock example"
subtitle: "M times Steepness Grid"
author: 
  L.T. Kell
...
vignette: >
  %\VignetteIndexEntry{Estimating M times Steepness Grid for Southern seabass stock}
...
opts_chunk$set(comment   =NA, 
               ...
               echo      =FALSE, 
               cache     =TRUE, 
               cache.path="../cache/South-Mh/",
               fig.path  ="../figs/South-Mh",
               fig.width =8,
               fig.height=6,
               dev       ="png")
...
## Scenarios for combinations of M and steepness (h)

```{r, scenarios}
path="C:/Users/mascar/Documents/Max/Commitees/ICES/WKBBASS/Southern stock/alternative_run_9c_Max"
scen=expand.grid(M=seq(0.4,2.0, 0.1),
                 h=c(0.8,  0.9, 0.999)) 
...
## Cretingt the scenarios folders
...
mScen=foreach(i=seq(dim(scen)[1]), 
    ...
    # Modify M vector and add manually in the control file #natm at the end of the M vector
    cFl  =scan(file.path(path,M,h,ctrl),what=as.character(),sep="\n")
    mVec=cFl[grep("# natm",tolower(cFl))[1]]
    mVec =as.numeric(unlist(strsplit(mVec, "\\s+")))
    mVec =mVec[!is.na(mVec)]*M
    cFl[grep("# natm",tolower(cFl))[1]]=paste(mVec,collapse=" ")
...
**Figure `r iFig=iFig+1; iFig`** Likelihood profile
...
scen=expand.grid(M=seq(0.4,2.0,0.2),
                 h=c(0.8,  0.9, 0.999)) 

dat=mlply(with(scen,file.path(path,M,h)),
               function(x)
                     tryIt(SS_output(x,verbose=FALSE,hidewarn=FALSE,printstat=FALSE)))
eql=llply(dat, function(x) tryIt(FLCandy:::curveSS(x,maxY=2.5))) 

eql=Map(function(x) cbind(x, scen[x$Scenario,]),
        FLCandy:::unnest(eql[!unlist(llply(eql,is.null))]))
